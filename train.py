...
with mp.solutions.holistic.Holistic(min_detection_confidence=0.75, min_tracking_confidence=0.75) as holistic:
    ...
    for action, sequence, frame in product(actions, range(sequences), range(frames)):
        ...
        if frame == 0: 
            while True:
                ...
                if not ret:
                    ...
                    continue  # Skip to the next iteration if the image capture failed
               ...
               # Display text on the image indicating the action and sequence number being recorded
                try:
                    ...
                    print(f"Error during putText: {e}")
        ...
        # Check if keypoints extraction was successful
        if keypoints.size == 0:
            print("No keypoints found, skipping saving.")
            continue

        frame_path = os.path.join(PATH, action, str(sequence), str(frame))
        np.save(frame_path, keypoints)
        try:
            print(f"Saved keypoints for action: {action}, sequence: {sequence}, frame: {frame}")
        except Exception as e:
